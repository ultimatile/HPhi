...
------------------------------------------
並列計算機に対応した数値厳密対角化法による有効模型ソルバーパッケージ。広汎な多体量子系の有効模型(多軌道ハバード模型、ハイゼンベルグ模型、近藤格子模型など)の基底状態及び低励起状態の波動関数を並列計算によって求められます。Lanczos法による基底状態計算、熱的純粋量子状態を利用した比熱・帯磁率の温度依存性計算が可能です。さらに、ver.2.0より数値ライブラリKωが接続され、先端的数理アルゴリズム(シフト型クリロフ部分空間理論)による動的グリーン関数の計算が可能となっています。
